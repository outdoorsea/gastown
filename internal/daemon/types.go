...
type PatrolsConfig struct {
	...
	EstopAutoTrigger       *EstopAutoTriggerConfig        `json:"estop_auto_trigger,omitempty"`
}

// EstopAutoTriggerConfig controls when the daemon automatically triggers an E-stop.
// All fields have sensible defaults; omitting the section entirely uses defaults.
type EstopAutoTriggerConfig struct {
	// Enabled controls whether auto-trigger is active. Default: true.
	Enabled *bool `json:"enabled,omitempty"`

	// DoltFailureThreshold is the number of consecutive Dolt health check
	// failures before triggering auto E-stop. Default: 3.
	DoltFailureThreshold int `json:"dolt_failure_threshold,omitempty"`

	// ProbeInterval is how often to check Dolt health during auto E-stop
	// for recovery detection. Default: "30s".
	ProbeInterval string `json:"probe_interval,omitempty"`

	// Cooldown is the minimum time between auto E-stop triggers.
	// Prevents rapid stop/resume cycles. Default: "5m".
	Cooldown string `json:"cooldown,omitempty"`

	// DiskUsagePercent triggers E-stop when disk usage exceeds this percentage.
	// Set to 0 to disable. Default: 95.
	DiskUsagePercent int `json:"disk_usage_percent,omitempty"`

	// DiskCheckInterval is how often to check disk usage. Default: "60s".
	DiskCheckInterval string `json:"disk_check_interval,omitempty"`
}

// GetDoltFailureThreshold returns the configured threshold or the default (3).
func (c *EstopAutoTriggerConfig) GetDoltFailureThreshold() int {
	if c != nil && c.DoltFailureThreshold > 0 {
		return c.DoltFailureThreshold
	}
	return 3
}

// IsEnabled returns whether auto-trigger is enabled (default: true).
func (c *EstopAutoTriggerConfig) IsEnabled() bool {
...
// GetCooldown returns the parsed cooldown duration or default (5m).
func (c *EstopAutoTriggerConfig) GetCooldown() time.Duration {
	if c != nil && c.Cooldown != "" {
...
// GetDiskUsagePercent returns the disk usage threshold or default (95).
// Returns 0 if disk checking is disabled.
func (c *EstopAutoTriggerConfig) GetDiskUsagePercent() int {
	if c != nil && c.DiskUsagePercent > 0 {
		return c.DiskUsagePercent
	}
	if c != nil && c.DiskUsagePercent == -1 {
